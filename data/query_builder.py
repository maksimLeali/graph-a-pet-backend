...
def build_simple_where(alias: str ,filters: Dict[str, dict] = {"fixeds": None, "lists": None, "ranges": None}, search: str = "", search_fields: list = [], join_filters: list =[]) -> str:
    filters_to_format = []
    ...
    return f"{'WHERE ' if len(formatted_filters) > 0 or len(search) >0 or len(join_filters) > 0  else ''}{ formatted_filters}"  \
...
def build_where(table: str ,filters: Dict[str, dict] = {"fixeds": None, "lists": None, "ranges": None}, search: str = "", search_fields: list = [], join_filters: list =[]) -> str:
